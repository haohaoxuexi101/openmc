...
- `examples/random_ray_solver/lessons/lesson03_region_crossing.cpp` 展示了如何在三维盒体集合中逐面计算离开距离，呼应 OpenMC `Cell::distance` 的实现策略。【F:examples/random_ray_solver/lessons/lesson03_region_crossing.cpp†L1-L52】
...
### 5.4 ParticleData 与 Cell 的接口契约

- 新增文档《OpenMC 粒子与几何对象接口深度解析》对 `particle_data.h` 与 `cell.h` 进行了逐字段拆解，解释为何 `ParticleData`
  要继承 `GeometryState`、`Cell` 如何通过 Region 布尔树暴露统一接口，以及 inline getter/setter 如何守护传输环的性能。【F:docs/particle_cell_interface_manual.md†L1-L98】
- `examples/courses/oop_design/lesson06_particle_data_bridge.cpp` 与 `lesson07_cell_interface_walkthrough.cpp` 将上述接口理念落实为可运行示例，帮助读者在自定义求解器中复用 OpenMC 的继承/组合策略。【F:examples/courses/oop_design/lesson06_particle_data_bridge.cpp†L1-L120】【F:examples/courses/oop_design/lesson07_cell_interface_walkthrough.cpp†L1-L138】
...
- `examples/random_ray_solver/lessons` 目录进一步将求解器拆分为六节课程：从域与材料配置、随机射线生成、区域穿越、碰撞物理、裂变银行管理到求解器驱动。读者可按顺序学习每节课，再回到完整代码进行整合。【F:examples/random_ray_solver/lessons/lesson01_domain_setup.cpp†L1-L144】【F:examples/random_ray_solver/lessons/lesson06_solver_driver.cpp†L1-L146】
...
- **碰撞调度器**：`physics.cpp` 的 `collision()` 与 `transport()` 例程统一从 `ParticleData` 获取截面、材料和坐标，再依据粒子类型调用 `sample_neutron_reaction()`、`sample_photon_reaction()` 等策略函数，体现“事件调度 + 类型分派”的解耦模式。【F:src/physics.cpp†L1-L101】
- **多群实现**：`physics_mg.cpp` 以 `collision_mg()`、`sample_reaction()`、`scatter()` 分解多群流程；函数体中通过 `Particle` 的 inline 接口访问能群、核数据缓存、随机流，避免额外适配层。【F:src/physics_mg.cpp†L26-L195】
- **生命周期控制**：`simulation.cpp` 在批次循环中交替调用 `initialize_batch()`、`execute_batch()`、`finalize_batch()`，并在每次碰撞后根据粒子状态更新统计量和裂变银行，形成“外层驱动 + 内核回调”的层次化设计。【F:src/simulation.cpp†L1-L115】

### 10.2 数据布局与内联接口

- `ParticleData` 通过继承 `GeometryState` 并组合 `neutron_xs_`、`macro_xs_` 等缓存，将所有热点字段存储为连续内存，内联 getter/setter 直接返回引用，避免函数调用开销。【F:include/openmc/particle_data.h†L421-L672】
- `GeometryState` 的 `coord_` 栈、`move_distance()`、`init_from_r_u()` 等函数同样以 `inline` 实现，使得几何与物理循环之间的状态同步开销降到最低。【F:include/openmc/particle_data.h†L223-L339】
- 结构体如 `NuclideMicroXS`、`MacroXS` 提前缓存插值索引和上次能量，搭配 `invalidate_neutron_xs()`，支持“只在能量或温度变化时重算”的懒更新策略。【F:include/openmc/particle_data.h†L102-L176】【F:include/openmc/particle_data.h†L649-L655】

### 10.3 几何搜索加速

- `geometry.cpp` 在 `find_cell_inner()` 中优先尝试邻接表（`NeighborList`）以 O(k) 搜索候选单元，失败时退回穷举；同时利用 `coord_` 栈递归下降宇宙与晶格，保证复杂几何仍保持局部性。【F:src/geometry.cpp†L102-L200】
- `cell_instance_at_level()` 根据父层 `offset_` 和晶格索引一次性计算分布式单元实例号，避免运行时多次遍历子层，大幅减少 tally 分区查找。【F:src/geometry.cpp†L66-L99】
- 邻接检测时借助 OpenMP 原子操作累计重叠计数，既保持线程安全，又让调试信息可用于几何校验。【F:src/geometry.cpp†L35-L62】

### 10.4 随机数与并行一致性

- `random_lcg.cpp` 基于 PCG 变种实现 64 位 LCG，并提供 `future_seed`、`init_particle_seeds` 等函数支持 O(log N) 的跨粒子跳跃，使 MPI/OpenMP 线程可按历史编号独立初始化而保持可重现性。【F:src/random_lcg.cpp†L7-L120】
- 粒子对象存储 `seeds_[N_STREAMS]` 与当前流索引，允许几何、物理、统计分别使用独立随机流，避免跨模块的相互干扰。【F:include/openmc/particle_data.h†L456-L593】
- `simulation::fission_bank.thread_safe_append()` 等接口在多线程环境下通过原子操作或锁策略收集裂变源，与 `ParticleData::nu_bank()` 的局部缓存形成“局部累积 + 全局归并”的并行模式。【F:src/physics_mg.cpp†L121-L177】【F:src/simulation.cpp†L78-L101】

### 10.5 函数级优化策略与实践建议

- **分支分离**：多群散射中的 `sample_reaction()` 先处理裂变、再处理吸收、最后调用散射，减少交叉依赖，利于编译器做指令预测；连续能量路径在 `physics.cpp` 内亦采用类似结构。【F:src/physics_mg.cpp†L43-L90】【F:src/physics.cpp†L1-L101】
- **稀疏访问规整化**：几何模块将旋转/平移提前应用在 `LocalCoord`，使后续 `Cell::contains()` 操作在局部坐标执行，从而降低浮点误差并提升缓存命中率。【F:src/geometry.cpp†L165-L200】【F:include/openmc/particle_data.h†L262-L307】
- **面向缓存的课程示例**：`examples/random_ray_solver` 通过独立课程拆解随机射线求解器的材料缓存、几何邻接、碰撞抽样，读者可在掌握 OpenMC 内核设计后仿写高性能应用。【F:examples/random_ray_solver/lessons/lesson06_solver_driver.cpp†L1-L146】【F:examples/random_ray_solver/main.cpp†L1-L786】

上述物理内核与高性能函数设计说明覆盖了从宏观层（批次调度）、中观层（几何与粒子接口）到微观层（内联函数、随机数）的完整链条，可作为编写高性能 Monte Carlo 求解器的参考蓝图。
...
- `examples/courses/parallel_computing` 课程从线程切分、MPI 归并一路拓展到非阻塞通信与 OpenMP 任务化负载均衡，新增的 lesson06/07 聚焦裂变银行流水线与工作窃取策略。【F:examples/courses/parallel_computing/lesson06_async_pipeline.cpp†L1-L47】【F:examples/courses/parallel_computing/lesson07_dynamic_tasking.cpp†L1-L43】
...
为配合手册“继承与组合”“输运算法”“并行化”章节，`examples/courses/` 目录提供了三套独立课程：

1. **面向对象架构课程（`oop_design/`）** —— 以 `GeometryState`、`Particle`、`Navigator` 等类为核心，逐课讲解继承树、组合关系、策略模式等接口设计技巧。
2. **输运算法课程（`transport_algorithms/`）** —— 覆盖射线追踪、碰撞抽样、特征值迭代、轨迹计分与多群核数据封装，帮助将理论步骤与代码实现一一对应。
3. **并行与高性能课程（`parallel_computing/`）** —— 讲解 OpenMP 粒子并行、MPI 裂变银行归并、线程私有随机源、混合并行通信与负载划分策略。

每套课程均包含不少于五个 Lesson，源码中配有中文注释说明“为何需要此接口/模式”。建议按课程顺序练习：

- 首先掌握面向对象课程，熟悉几何、粒子与计分的类关系。
- 接着完成输运算法课程，将物理步骤映射到抽样流程。
- 最后攻克并行课程，理解 OpenMC 如何扩展到多线程与多进程环境。

所有 Lesson 可使用 `g++ -std=c++17`、`-fopenmp` 或 `mpicxx` 直接编译运行，具体指令详见 `examples/courses/README.md`。

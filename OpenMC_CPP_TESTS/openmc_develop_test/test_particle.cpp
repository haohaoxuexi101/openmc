...
int main()
{
  ...
  std::cout << "Particle speed (neutron, 1 MeV): " << particle_speed << " m/s"
            << std::endl;
  ...
  if (particle.n_coord() > 0) {
    std::cout << "Particle coordinate levels initialized: "
              << particle.n_coord() << std::endl;
    ...
    std::cerr << "Error: Particle coordinate not properly initialized!"
              << std::endl;
    return -1;
  }

  // 打印初始位置
  std::cout << "Initial particle position: (" << particle.coord(0).r.x << ", "
            << particle.coord(0).r.y << ", " << particle.coord(0).r.z << ")"
            << std::endl;
  ...
  // 检查粒子是否在 x 轴上移动
  assert(particle.coord(0).r.x == 1.0 &&
         "Particle should have moved 1 meter along x-axis");

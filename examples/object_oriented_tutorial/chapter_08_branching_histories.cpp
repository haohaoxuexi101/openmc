...
int main() {
  run_chapter_simulation("Fission weight amplification", 10, [](Particle& particle, TallyResult& tally) {
    ...
    if (step >= collision - 1e-12) {
      ...
      ++tally.collisions;
      if (interaction == Material::Interaction::Fission) {
        tally.fission += particle.data().weight;
        std::cout << "Fission branch weight now " << particle.data().weight << '\n';
      }

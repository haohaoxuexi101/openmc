...
| Lesson | 主题 | 亮点 | OpenMC 对照 |
| --- | --- | --- | --- |
| lesson01_geometry_state.cpp | 几何状态建模 | Surface/Cell 继承 + LessonGeometryState 组合 | 对照 `src/geometry/surface.h` 与 `src/geometry/cell.h` 的接口骨架 |
| lesson02_particle_lifecycle.cpp | 粒子状态生命周期 | move/scatter/absorb 接口守护状态一致性 | 借鉴 `src/particle_data.h` 的粒子字段布局 |
| lesson03_navigation_context.cpp | 定位上下文解耦 | Context 持有 Navigator，演示接口分离 | 对照 `src/geometry/navigator.h` 的定位调用顺序 |
| lesson04_material_binding.cpp | 几何-材料组合 | MaterialRegistry 绑定 Cell 材料 | 对照 `src/geometry/cell.h` 的 material_ 与 `src/material.cpp` 的注册模式 |
| lesson05_interface_strategy.cpp | 策略模式 | 计分策略抽象，映射 Tally 设计 | 借鉴 `src/tallies/tally.h` 的策略化 Score 结构 |
| lesson06_particle_data_bridge.cpp | ParticleData 接口桥接 | 解释 GeometryStateBridge 继承 + 状态字段布局 | 对照 `src/particle_data.h` 的 GeometryState/ParticleData 继承关系 |
| lesson07_cell_interface_walkthrough.cpp | Cell 布尔树拆解 | Region 多态与 distance 接口详解 | 借鉴 `src/geometry/cell.h` 的 Region 布尔树与距离计算入口 |

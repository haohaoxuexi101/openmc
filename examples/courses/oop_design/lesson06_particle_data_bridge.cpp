...
// -----------------------------------
// 借鉴说明：ParticleDataLite 的字段布局直接取材于 OpenMC `src/particle_data.h`
// 中 Particle、ParticleData 的组合关系；GeometryStateBridge 则映射文件内的
// GeometryState 基类。为保持教学代码独立，类名采用 Bridge/Lite 后缀且不再
// 放置于嵌套命名空间。

// ------------------ 几何状态基类（对应 geometry.h） ------------------
class GeometryStateBridge {
public:
  struct Coord {
    double x = 0.0;
    double y = 0.0;
  ...
  GeometryStateBridge() = default;
  explicit GeometryStateBridge(Coord r) : r_(r) {}
...
class ParticleDataLite : public GeometryStateBridge {
public:
  ParticleDataLite() = default;

  ParticleDataLite(uint64_t id, Coord r, std::array<uint64_t, 4> seeds)
      : GeometryStateBridge(r), id_(id), seeds_(seeds) {}

...
// 本示例演示如何在 C++ 中以 OpenMC 风格抽象几何状态：
// 1. Surface/Cell 采用继承树表达差异化行为
// 2. LessonGeometryState 组合上述对象，负责追踪粒子的空间位置
// 3. 每个类使用中文注释，说明接口设计与职责边界
// 特别说明：为了避免与 OpenMC `particle_data.h` 中的 GeometryState 产生命名冲突，
// 本示例所有教学类均放在 lesson::oop::geometry_state 命名空间下，并使用
// LessonGeometryState 作为精简实现名称，表明它是教学版、非源码原型。

namespace lesson {
namespace oop {
namespace geometry_state {
...
int main() {
  using namespace lesson::oop::geometry_state;

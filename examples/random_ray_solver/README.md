...
> **新增：模块化课程**
>
> `lessons/` 子目录提供 6 节配套教学代码，分别拆解域构建、射线抽样、
> 区域穿越、碰撞物理、裂变银行与主循环驱动。建议先逐个编译运行课程，
> 再阅读完整求解器的实现，以形成“接口 -> 模块 -> 系统”的学习闭环。
...
| 课程文件 | 学习目标 |
| --- | --- |
| `lesson01_domain_setup.cpp` | 解析材料/几何配置，建立区域与材料映射 |
| `lesson02_ray_generation.cpp` | 均匀体源采样与各向同性方向生成 |
| `lesson03_region_crossing.cpp` | 计算射线离开盒体的距离与撞击面 |
| `lesson04_collision_physics.cpp` | 多群碰撞类型抽样与贡献因子计算 |
| `lesson05_bank_management.cpp` | 裂变银行归一化与下一代源项再生 |
| `lesson06_solver_driver.cpp` | 组合上述模块，实现最小随机射线求解循环 |

编译示例：

```bash
g++ -std=c++17 -O2 -o lesson_rr1 lessons/lesson01_domain_setup.cpp
g++ -std=c++17 -O2 -o lesson_rr6 lessons/lesson06_solver_driver.cpp
```

这些课程与《OpenMC 粒子与几何对象接口深度解析》说明书相互对应，
帮助读者将 `particle_data.h`、`cell.h` 的接口设计与随机射线求解器的实现进行对照理解。

...
| `lessons/lesson_oop_architecture.cpp` | 梳理 Surface/Cell/Geometry/Particle 的继承与组合接口设计。 | `g++ -std=c++17 -O2 -o lesson_oop lessons/lesson_oop_architecture.cpp` |
...
| `lessons/lesson_transport_algorithm.cpp` | 复刻 OpenMC 的多群输运循环、裂变银行与 k-effective 计算。 | `g++ -std=c++17 -O2 -o lesson_transport lessons/lesson_transport_algorithm.cpp` |
| `lessons/lesson_parallel_driver.cpp` | 演示多线程粒子分片、线程本地 RNG 与并行裂变重采样。 | `g++ -std=c++17 -O2 -pthread -o lesson_parallel lessons/lesson_parallel_driver.cpp` |

> 进阶学习者可进一步查阅 `examples/courses/` 目录，那里将**面向对象设计、输运算法、并行高性能**三大主题拆分为三套独立课程，每套不少于五节 Lesson，并配套 README 说明学习路线。面向对象课程新增了聚焦 `particle_data.h` 与 `cell.h` 的第 6、7 课，可配合《OpenMC 粒子与几何对象接口深度解析》说明书阅读源码。

所有小程序均可直接运行，并在控制台输出关键步骤，帮助读者由浅入深掌握 OpenMC 风格的
面向对象实现。若需完整的三维随机射线多群求解器，请参见 `examples/random_ray_solver` 目
录。

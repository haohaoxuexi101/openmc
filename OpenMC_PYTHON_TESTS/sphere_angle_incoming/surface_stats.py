...
def plot_direction_field_3d(numeric: dict, output_dir: str, sample: int = 300,
                            length: float = 0.2, linewidth: float = 1.2):
    r = np.vstack((numeric['r.x'], numeric['r.y'], numeric['r.z'])).T
...
def plot_direction_slice_2d(numeric: dict, output_dir: str,
                            sample: int = 1500, z_tol: float = 0.005):
    r = np.vstack((numeric['r.x'], numeric['r.y'], numeric['r.z'])).T
    u = np.vstack((numeric['u.x'], numeric['u.y'], numeric['u.z'])).T
    zc = np.median(r[:,2])
    ...
    ax.quiver(rs[:,0], rs[:,1], us[:,0], us[:,1], angles='xy', scale_units='xy', scale=1.5, width=0.003)
